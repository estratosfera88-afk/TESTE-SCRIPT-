-- [[
--     AKAT SCRIPT - THE STRONGEST BATTLEGROUNDS (TBS) [v5.0]
--     DELTA MOBILE 2026 EDITION - FULL BUG FIX & OPTIMIZED
--     Correções: Drag mobile, PhysicalProperties, HitboxReach,
--                AutoBlock, AutoTech, HttpGet, Toggles one-shot,
--                BlurEffect cleanup, pcall guards, RenderStepped → Heartbeat
-- ]]
...
local UI_TEXT = {
    ...
    CancelBtn         = "Cancelar",
    Intro             = '<font color="#FFFFFF">Scripts por | </font><font color="#8B0000">AKAT TBS Edition</font>',
    Tabs = {
        Player    = "Jogador",
        Combat    = "Combate",
        Visuals   = "Visuais",
    ...
    Options = {
        ...
        AutoBlock   = { Title = "AutoBlock",    Desc = "Detecta golpes inimigos e bloqueia automaticamente." },
        AutoTech    = { Title = "AutoTech",     Desc = "Executa recuperação (Tech) instantaneamente ao cair." },
        HitboxReach = { Title = "HitboxReach",  Desc = "Aumenta o alcance dos seus ataques M1 à distância." },
        AimLock     = { Title = "AimLock",      Desc = "Carrega o script de trava de mira externo para TBS." },
        HitboxESP   = { Title = "HitboxESP",    Desc = "Exibe a caixa de colisão/ataque dos adversários." },
        TpArena     = { Title = "TpArena",      Desc = "Teleporta instantaneamente para o centro da arena." },
        SafeSpot    = { Title = "SafeSpot",     Desc = "Cria e te teleporta para uma plataforma segura no céu." },
        TpTarget    = { Title = "TpTarget",     Desc = "Teleporta para as costas do jogador mais próximo." },
        AntiFling   = { Title = "AntiFling",    Desc = "Desativa colisões com outros jogadores." },
...
screenGui.Name       = "AkatTBSUniversalUI"
screenGui.ResetOnSpawn = false
screenGui.IgnoreGuiInset = true

-- BUG FIX: Seleção segura do pai da UI (Delta Mobile pode não ter CoreGui acessível)
...
-- Limpa UI anterior de forma segura
local oldGui = uiParent:FindFirstChild("AkatTBSUniversalUI")
if oldGui then pcall(function() oldGui:Destroy() end) end

screenGui.Parent = uiParent

-- ============================================================
-- [SISTEMA DE TRANSPARÊNCIA SINCRONIZADA]
-- ============================================================
...
local function AplicarFadeSincronizado(raiz, fadeOut, duracao)
    ...
    local function tratarObjeto(obj)
        ...
        pcall(function()
            ...
            if orig.TextStrokeTransparency ~= nil then
                local t = fadeOut and 1 or orig.TextStrokeTransparency
                if duracao == 0 then obj.TextStrokeTransparency = t
                else TweenService:Create(obj, info, {TextStrokeTransparency = t}):Play() end
            end
...
-- ============================================================
-- [DRAG — CORRIGIDO PARA DELTA MOBILE 2026]
-- BUG FIX: currentInput.Changed não funciona bem no mobile.
-- Substituído por UserInputService.InputEnded com verificação de ID.
-- ============================================================
local function ConfigurarArrastarAkat(inst, trigger)
    ...
    trigger.InputBegan:Connect(function(input)
        ...
        if (isTouch or isMouse) and not dragging then
            dragging         = true
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        local isTouch = input.UserInputType == Enum.UserInputType.Touch
        if isMove or (isTouch and activeInputObject and input == activeInputObject) then
    ...
    -- BUG FIX: usa InputEnded global em vez de currentInput.Changed
...
-- ============================================================
-- [BOTÃO FLUTUANTE]
-- BUG FIX: Visible = true direto; FloatBtn posicionado corretamente mobile
-- ============================================================
...
FloatBtn.Image            = "rbxassetid://7733960981" -- ícone genérico confiável (sem rbxthumb que pode falhar)
FloatBtn.ImageColor3      = Color3.fromRGB(255, 255, 255)
FloatBtn.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
FloatBtn.Visible          = false   -- controlado pela intro
FloatBtn.ZIndex           = 30
...
TweenService:Create(StrokeGradient, TweenInfo.new(4, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1), {Rotation = 360}):Play()
...
mainWrapper.Position         = UDim2.new(0.5, 0, 0.5, -160)
...
subtitle.Text           = "TBS EDITION [v5.0 Delta]"
...
Instance.new("UICorner", SidebarBgContainer).CornerRadius = UDim.new(0, 9)
...
local function CriarIconeProcedural(parent, tabName)
    ...
    local icons = {
        Player    = "rbxassetid://78324938264014",
        Teleports = "rbxassetid://122367250674432",
        Misc      = "rbxassetid://79429182159899",
        Visuals   = "rbxassetid://135604583195835",
        Combat    = "rbxassetid://139442231247295"
    }
    imageLabel.Image = icons[tabName] or ""
end
...
local function createTabBtn(tabName)
    ...
    tabBtn.MouseLeave:Connect(function()
        ...
        if activeTab ~= tabName then
            TweenService:Create(tabBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(12, 12, 12), BackgroundTransparency = 1}):Play()
            TweenService:Create(tabLabel, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(180, 180, 180)}):Play()
            RecolorirIcone(tabBtn:FindFirstChild("Icon"), Color3.fromRGB(180, 180, 180), TweenInfo.new(0.15))
        end
    end)
    tabBtn.MouseEnter:Connect(function()
        if activeTab ~= tabName then
            TweenService:Create(tabBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(22, 22, 22), BackgroundTransparency = 0.7}):Play()
            TweenService:Create(tabLabel, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(220, 220, 220)}):Play()
            RecolorirIcone(tabBtn:FindFirstChild("Icon"), Color3.fromRGB(220, 220, 220), TweenInfo.new(0.15))
        end
    end)
...
-- [VISUAL DOS TOGGLES]
-- ============================================================
local function UpdateToggleVisual(configKey, state)
    local elem = toggleElements[configKey]
    if elem then
        local targetPos   = state and UDim2.new(1, -17, 0.5, -7) or UDim2.new(0, 3, 0.5, -7)
        local targetColor = state and Color3.fromHex("#8B0000") or Color3.fromRGB(30, 30, 30)
        local anim        = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        TweenService:Create(elem.Circle, anim, {Position     = targetPos  }):Play()
        TweenService:Create(elem.Track,  anim, {BackgroundColor3 = targetColor}):Play()
    end
end

-- ============================================================
-- [CRIAÇÃO DE TOGGLES]
-- BUG FIX: Toggles one-shot (TpArena, TpTarget) agora têm estilo de botão
-- ============================================================
local ONE_SHOT_KEYS = { TpArena = true, TpTarget = true }
...
local function createToggle(parent, configKey, tabCategory)
    ...
    if ONE_SHOT_KEYS[configKey] then
        -- Botão de ação única (sem switch visual)
        local actionBtn      = Instance.new("Frame", toggleFrame)
        actionBtn.Size       = UDim2.new(0, 60, 0, 24)
        actionBtn.Position   = UDim2.new(1, -72, 0.5, -12)
        actionBtn.BackgroundColor3 = Color3.fromHex("#8B0000")
        actionBtn.BorderSizePixel  = 0
        actionBtn.ZIndex     = 6
        Instance.new("UICorner", actionBtn).CornerRadius = UDim.new(0, 6)

        local btnLabel       = Instance.new("TextLabel", actionBtn)
        ...
        triggerBtn.MouseButton1Click:Connect(function()
            TweenService:Create(actionBtn, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(180, 0, 0)}):Play()
            task.delay(0.15, function()
                TweenService:Create(actionBtn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromHex("#8B0000")}):Play()
            end)
            if _G.AkatCallbacks and _G.AkatCallbacks[configKey] then
                task.spawn(function()
                    pcall(_G.AkatCallbacks[configKey], true)
                end)
            end
        end)
    else
        -- Toggle normal com switch
        ...
        toggleElements[configKey] = { Track = switchTrack, Circle = switchCircle }
        ...
        triggerBtn.MouseButton1Click:Connect(function()
            ...
            UpdateToggleVisual(configKey, Configs[configKey])
            if _G.AkatCallbacks and _G.AkatCallbacks[configKey] then
                task.spawn(function()
...
-- ============================================================
-- [ALTERNÂNCIA DO POPUP DE CONFIRMAÇÃO]
-- ============================================================
    ...
    if exibir then
        ...
        if confirmBlur then
            ...
            blurTween.Completed:Connect(function()
                if confirmBlur and confirmBlur.Parent and confirmBlur.Size == 0 then
        ...
        task.delay(tempoAnim, function()
            if not isConfirmOpen then confirmFrame.Visible = false end
        end)
    end
end
...
local function executarMinimizacao()
    ...
    if isMinimized then
        ...
        -- BUG FIX: ReleaseFocus pode crashar no Delta se não houver TextBox focado
...
-- BUG FIX: Blur e IntroFrame destruídos em pcall para evitar leak
-- ============================================================
local function ExecutarIntroAkat()
    ...
    -- BUG FIX: Destruir blur da intro com segurança
    ...
    openTween.Completed:Connect(function()
        selectTab("Player")
        pcall(function() MainScale:Destroy() end)
        pcall(function() IntroFrame:Destroy() end)
        CriarNotificacao("AKAT TBS v5.0", "Carregado com Sucesso no Delta!", 4)
    end)
end

-- ============================================================
-- [EFEITO HOVER NOS BOTÕES DO TOPO]
-- ============================================================
local function AplicarEfeitoFisicoBotao(btn, hoverColor)
    btn.MouseEnter:Connect(function()
        ...
        pcall(function()
            if btn.Name == "MinimizeBtn" then
                TweenService:Create(btn.Line, TweenInfo.new(0.15), {BackgroundColor3 = hoverColor}):Play()
            elseif btn.Name == "SearchBtn" then
                TweenService:Create(circleStroke, TweenInfo.new(0.15), {Color = hoverColor}):Play()
                TweenService:Create(SearchHandle, TweenInfo.new(0.15), {BackgroundColor3 = hoverColor}):Play()
            elseif btn.Name == "CloseBtn" then
                TweenService:Create(btn.Line1, TweenInfo.new(0.15), {BackgroundColor3 = hoverColor}):Play()
                TweenService:Create(btn.Line2, TweenInfo.new(0.15), {BackgroundColor3 = hoverColor}):Play()
            end
        end)
    end)
    btn.MouseLeave:Connect(function()
        ...
        pcall(function()
            if btn.Name == "MinimizeBtn" then
                TweenService:Create(btn.Line, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromHex("#A0A0A0")}):Play()
            elseif btn.Name == "SearchBtn" then
                TweenService:Create(circleStroke, TweenInfo.new(0.15), {Color = Color3.fromHex("#A0A0A0")}):Play()
                TweenService:Create(SearchHandle, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromHex("#A0A0A0")}):Play()
            elseif btn.Name == "CloseBtn" then
                TweenService:Create(btn.Line1, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromHex("#A0A0A0")}):Play()
                TweenService:Create(btn.Line2, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromHex("#A0A0A0")}):Play()
            end
        end)
    end)
end
...
createToggle(togglesContainer, "AutoBlock",   "Combat")
createToggle(togglesContainer, "AutoTech",    "Combat")
createToggle(togglesContainer, "HitboxReach", "Combat")
createToggle(togglesContainer, "AimLock",     "Combat")

createToggle(togglesContainer, "HitboxESP",   "Visuals")

createToggle(togglesContainer, "TpArena",     "Teleports")  -- one-shot
createToggle(togglesContainer, "SafeSpot",    "Teleports")
createToggle(togglesContainer, "TpTarget",    "Teleports")  -- one-shot

createToggle(togglesContainer, "AntiFling",   "Misc")
createToggle(togglesContainer, "AutoEmote",   "Misc")
...
SearchBtn.MouseButton1Click:Connect(function()
    ...
    if searchOpen then
        ...
        -- BUG FIX: pcall para evitar crash no Delta Mobile
        ...
        filterToggles(activeTab, "")
        tween.Completed:Connect(function()
            if not searchOpen then searchBarFrame.Visible = false end
        end)
    end
end)
...
CloseBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        task.delay(0.2, function() AlternarConfirmacao(true) end)
    else
...
btnYes.MouseButton1Click:Connect(function()
    local syncTime = 0.18
    if confirmBlur then
        TweenService:Create(confirmBlur, TweenInfo.new(syncTime, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Size = 0}):Play()
    end
...
local function AnimarCliqueFloatBtn()
    local orig = UDim2.new(0, 44, 0, 44)
    local tgt  = UDim2.new(0, 41, 0, 41)
    local shrink = TweenService:Create(FloatBtn, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = tgt})
    local expand = TweenService:Create(FloatBtn, TweenInfo.new(0.1,  Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = orig})
    shrink:Play()
...
-- ============================================================
-- [LÓGICA DE COMBATE — THE STRONGEST BATTLEGROUNDS]
-- ============================================================
local connections  = {}
local safePlatform = nil

-- BUG FIX: Verificação robusta de HumanoidRootPart antes do magnitude
...
-- Tamanhos originais para restaurar HitboxReach e HitboxESP
local originalHandSize = Vector3.new(1, 2, 1)
...
_G.AkatCallbacks = {
    ...
    -- --------------------------------------------------------
    -- BUG FIX: PhysicalProperties.new requer 5 argumentos obrigatórios:
    -- (density, friction, elasticity, frictionWeight, elasticityWeight)
    InfDash = function(state)
        if connections["InfDash"] then
            connections["InfDash"]:Disconnect()
            connections["InfDash"] = nil
        end
        if state then
            connections["InfDash"] = RunService.Stepped:Connect(function()
                pcall(function()
                    local char = player.Character
                    if char then
                        local props = PhysicalProperties.new(0.7, 0.3, 0.5, 1.0, 1.0)
    ...
    -- --------------------------------------------------------
    -- BUG FIX: AutoBlock — FireServer corrigido; verifica Communicator de forma mais robusta
                ...
                pcall(function()
                    ...
                    for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                        if track.IsPlaying then
                            ...
                            if n:find("m1") or n:find("attack") or n:find("punch") or n:find("kick") or n:find("swing") then
                                local myChar = player.Character
                                if myChar then
                                    -- TBS usa RemoteEvent chamado "RE" ou "Communicator" dependendo da versão
                                    local comm = myChar:FindFirstChild("Communicator")
                                         or myChar:FindFirstChild("RE")
                                    if comm and comm:IsA("RemoteEvent") then
                                        pcall(function()
                                            comm:FireServer("Block", true)
                                        end)
                                    end
                                end
                                break
    ...
    -- --------------------------------------------------------
    -- BUG FIX: AutoTech — lógica mais robusta; detecta ragdoll pelo HumanoidState
    AutoTech = function(state)
        if connections["AutoTech"] then
            connections["AutoTech"]:Disconnect()
            connections["AutoTech"] = nil
        end
        if state then
            connections["AutoTech"] = RunService.Heartbeat:Connect(function()
                pcall(function()
                    ...
                    -- Detecta ragdoll/knocked pelos estados de física
    ...
    -- --------------------------------------------------------
    -- BUG FIX: HitboxReach — salva tamanho real antes de alterar; restaura corretamente
    HitboxReach = function(state)
        if connections["HitboxReach"] then
            connections["HitboxReach"]:Disconnect()
            connections["HitboxReach"] = nil
        end
        if state then
            -- Salva tamanho original na primeira vez
            local char = player.Character
            if char then
                local rHand = char:FindFirstChild("RightHand") or char:FindFirstChild("Right Arm")
                if rHand then originalHandSize = rHand.Size end
            end
            connections["HitboxReach"] = RunService.Heartbeat:Connect(function()
                pcall(function()
                    local myChar = player.Character
                    if myChar then
                        local rHand = myChar:FindFirstChild("RightHand") or myChar:FindFirstChild("Right Arm")
                        if rHand then
                            rHand.Size         = Vector3.new(12, 12, 12)
                            rHand.Transparency = 0.8
    ...
    -- --------------------------------------------------------
    -- BUG FIX: AimLock — HttpGet protegido + fallback de erro claro
    AimLock = function(state)
        if state then
            task.spawn(function()
                local ok, err = pcall(function()
                    local src = game:HttpGet("https://raw.githubusercontent.com/estratosfera88-afk/AIM-LOCK-TBS/refs/heads/main/main.lua", true)
                    loadstring(src)()
                end)
                if ok then
                    CriarNotificacao("Aim Lock", "Script de Aim Lock carregado!", 3)
                else
                    CriarNotificacao("Aim Lock", "Falha ao carregar: " .. tostring(err):sub(1, 40), 4)
                    -- Reverte toggle
                    task.delay(0.1, function()
                        Configs.AimLock = false
                        UpdateToggleVisual("AimLock", false)
                    end)
                end
            end)
        end
    ...
    -- --------------------------------------------------------
    -- BUG FIX: HitboxESP — salva tamanho original; usa Heartbeat em vez de RenderStepped
                    ...
                    for _, p in ipairs(Players:GetPlayers()) do
                        if p ~= player and p.Character then
                            ...
                            if hrp then
                                ...
                                hrp.Color        = Color3.fromRGB(139, 0, 0)
            ...
            pcall(function()
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= player and p.Character then
                        local hrp = p.Character:FindFirstChild("HumanoidRootPart")
                        if hrp then
                            -- BUG FIX: restaura tamanho correto do HRP
    ...
    -- --------------------------------------------------------
    -- BUG FIX: TpArena é now one-shot via botão; sem reset de toggle necessário
    TpArena = function(_)
        pcall(function()
            local myChar = player.Character
            if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                myChar.HumanoidRootPart.CFrame = CFrame.new(0, 50, 0)
                CriarNotificacao("Teleporte", "Teleportado para a Arena!", 2)
            end
        end)
    end,

    -- --------------------------------------------------------
    SafeSpot = function(state)
        if state then
            pcall(function()
                if not safePlatform or not safePlatform.Parent then
                    ...
                    safePlatform.Color          = Color3.fromRGB(139, 0, 0)
                ...
                local myChar = player.Character
                if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                    myChar.HumanoidRootPart.CFrame = CFrame.new(0, 805, 0)
                end
            end)
        else
            pcall(function()
    ...
    -- --------------------------------------------------------
    -- BUG FIX: TpTarget é now one-shot via botão; lógica mantida
    TpTarget = function(_)
        pcall(function()
            local target = GetClosestTarget(300)
            if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                local myChar = player.Character
                if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                    myChar.HumanoidRootPart.CFrame = target.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3)
                    CriarNotificacao("Teleporte", "Teleportado para " .. target.Name, 2)
                end
            else
                CriarNotificacao("TpTarget", "Nenhum jogador próximo.", 2)
            end
        end)
    end,

    -- --------------------------------------------------------
    AntiFling = function(state)
    ...
    -- --------------------------------------------------------
    AutoEmote = function(state)
        if connections["AutoEmote"] then
            connections["AutoEmote"]:Disconnect()
            connections["AutoEmote"] = nil
        end
        if state then
            connections["AutoEmote"] = UserInputService.InputBegan:Connect(function(input, gameProcessed)
                    ...
                    pcall(function()
                        ...
                        if myChar then
                            ...
                            if hum then
                                for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                                    local n = track.Name:lower()
                                    if n:find("dance") or n:find("emote") then
                                        track:Stop()
                                    end
                                end
                            end
    ...
    ShutdownAll = function()
        ...
        -- Restaura propriedades alteradas
        pcall(function()
            local myChar = player.Character
            if myChar then
                local hum   = myChar:FindFirstChildOfClass("Humanoid")
...
-- Registra transparências do confirmFrame antes da intro
pcall(function()
    RegistrarTransparencias(confirmFrame)
    for _, d in ipairs(confirmFrame:GetDescendants()) do
        RegistrarTransparencias(d)
    end
end)
