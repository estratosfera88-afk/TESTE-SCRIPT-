-- [[
--     AKAT MM2 SCRIPT [BETA v3.2] - ESP + AUTO SHOOT REWRITE 2026
-- ]]
...
local Locales = {
    ...
    ES = {
        ...
        Options = {
            ...
            ESP = { Title = "ESP Jogadores", Desc = "Resalta jugadores por rol (Sheriff Azul / Héroe Amarillo)." },
...
local function AS_Tick()
    ...
    -- [MOBILE FIX] Rotaciona o personagem instantaneamente para o alvo
    myRoot.CFrame = CFrame.new(myRoot.Position, Vector3.new(targetPos.X, myRoot.Position.Y, targetPos.Z))

    -- [MOBILE FIX] Câmera acompanha usando Lerp suave de 0.18
...
-- [BOTÃO FLUTUANTE DE CONFIGURAÇÕES - MENU]
...
-- [BOTÃO FLUTUANTE DE AUTO SHOOT PARA MOBILE]
...
-- Execução do clique de Auto Shoot Mobile
asBtnText.MouseButton1Click:Connect(function()
    -- Efeito tátil pop de clique
...
-- [ESTRUTURA GERAL DA JANELA DO MENU]
...
mainFrame.BackgroundTransparency = 0.22 -- [PROPORÇÃO TRANSPARENTE SOLICITADA]
...
-- [SISTEMA DE SIDEBAR TRANSPARENTE]
...
SidebarFrame.BackgroundTransparency = 0.35 -- [TRANSPARÊNCIA ADICIONADA]
...
-- [TABS LIST SYSTEM - INTEGRADAS COMO LISTA EMPILHADA]
...
TabsLayout.Padding = UDim.new(0, 0) -- [PADDING 0 PARA FICAR EM CIMA DA OUTRA COMO LISTA]
...
local function createTabBtn(tabName)
    ...
    tabBtn.Size = UDim2.new(1, 0, 0, 36) -- Largura total preenchendo a sidebar
    ...
    -- [EFEITO SELECIONADO LATERAL DA LISTA]
...
-- Callbacks dinâmicos de Toggles
...
-- ==================== FUNÇÕES AUXILIARES TELEPORT E AUTOCOLLECT ====================
...
hbConnection = RunService.Heartbeat:Connect(function(dt)
    ...
    if Configs.Reach then
        ...
        if myKnife then
            ...
            if handle then
                -- 1. Cria o invólucro visual sutil do Reach
                ...
                -- 2. Sistema de Hitbox Expansion via firetouchinterest de alta velocidade
                for _, p in ipairs(Players:GetPlayers()) do
        ...
        -- Remove resquícios visuais do Reach ao desligar
        for _, item in ipairs(char:GetChildren()) do
            if item:IsA("Tool") then
                local handle = item:FindFirstChild("Handle")
                local rp = handle and handle:FindFirstChild("AkatReachPart")
