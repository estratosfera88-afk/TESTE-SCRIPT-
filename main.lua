-- =====================================================================
-- [[ AKATSUKI BLOX FRUITS SCRIPT [v1.2.0] - AUTO FARM | PVP | RAID | TP ]]
-- =====================================================================
...
local farmHoverHeight = 35  -- altura flutuante acima do mob (studs)
local farmAttackRate  = 0.08 -- intervalo de ataque (segundos)
...
-- Retorna TODOS os inimigos vivos, ordenados por distância
local function GetAllEnemies()
    local list = {}
    if not CharacterReady() then return list end
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and obj ~= character then
            ...
            if hum and hrp and hum.Health > 0 and not Players:GetPlayerFromCharacter(obj) then
                local d = (hrp.Position - humanoidRootPart.Position).Magnitude
                table.insert(list, { Model = obj, HRP = hrp, Humanoid = hum, Dist = d })
            end
        end
    end
...
-- Equipa a melhor ferramenta disponível
...
-- Dispara remotes de dano invisível ao estilo Redz Hub
local function FireDamageRemotes(enemyModel)
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if not remotes then return end
        -- Tenta vários nomes comuns de remote de dano no BF
        local candidates = { "Validator", "CDN", "CommD_", "HitEvent" }
        for _, name in ipairs(candidates) do
            local r = remotes:FindFirstChild(name)
            if r then
                if r:IsA("RemoteFunction") then
                    pcall(function() r:InvokeServer(enemyModel) end)
                elseif r:IsA("RemoteEvent") then
                    pcall(function() r:FireServer(enemyModel) end)
                end
            end
        end
    end)
...
-- ==================== AUTO FARM (LÓGICA REDZ HUB) ====================
--[[
    Comportamento:
    1. Equipa ferramenta
    2. Pega missão atual do jogador
    3. Encontra o inimigo mais próximo do tipo da missão
    4. Flutua a farmHoverHeight studs ACIMA do mob e PARA (não fica seguindo)
    5. Ativa a ferramenta e dispara remotes de dano em loop rápido
    6. Quando o mob morre, vai para o próximo
    7. Quando a missão termina, pega nova missão e continua
]]
local function StartAutoFarm()
    -- Para qualquer farm anterior com segurança
    farmLoop = false
    if farmThread then
        task.wait(0.2)
    end
    ...
    farmThread = task.spawn(function()
        ...
        -- Garante ferramenta equipada antes de começar
        ...
        while farmLoop and Configs.AutoFarm do
            ...
            -- Tenta pegar/renovar missão via remote padrão do BF
            pcall(function()
                local remotes = ReplicatedStorage:FindFirstChild("Remotes")
                if remotes then
            ...
            if not enemy then
                -- Nenhum mob encontrado, aguarda spawn
                task.wait(1)
                continue
            end
            ...
            -- === FASE 1: TELEPORTA UMA VEZ PARA CIMA DO MOB E PARA ===
            local hoverPos = hrp.Position + Vector3.new(0, farmHoverHeight, 0)
            SafeTeleport(hoverPos)

            -- Trava velocidade para não cair / deslizar
            humanoidRootPart.AssemblyLinearVelocity  = Vector3.zero
            humanoidRootPart.AssemblyAngularVelocity = Vector3.zero

            -- === FASE 2: LOOP DE ATAQUE ENQUANTO O MOB VIVE ===
            while farmLoop and Configs.AutoFarm and enemyHum and enemyHum.Health > 0 do
                if not CharacterReady() then break end

                -- Mantém posição travada acima do mob (recalcula se mob andou)
                if hrp and hrp.Parent then
                    local targetHover = hrp.Position + Vector3.new(0, farmHoverHeight, 0)
                    -- Só re-teleporta se afastou demais (mob se moveu muito)
                    local drift = (humanoidRootPart.Position - targetHover).Magnitude
                    if drift > 8 then
                        SafeTeleport(targetHover)
                    end
                    humanoidRootPart.AssemblyLinearVelocity  = Vector3.zero
                    humanoidRootPart.AssemblyAngularVelocity = Vector3.zero
                else
                    break -- hrp sumiu, mob morreu
                end

                -- Ataque invisível rápido
                pcall(function() tool:Activate() end)
                FireDamageRemotes(enemy.Model)

                task.wait(farmAttackRate)
            end

            -- Mob morreu ou saiu do mapa — pequena pausa antes do próximo
            task.wait(0.15)
        end
    end)
end
            ...
            if CharacterReady() then
                ...
                for i = 1, 60 do
                    ...
                    if CharacterReady() then
                        local enemy = GetClosestEnemy()
                        if enemy then
                            SafeTeleport(enemy.HRP.Position + Vector3.new(0, farmHoverHeight, 0))
                            task.wait(0.1)
                            local tool = character:FindFirstChildOfClass("Tool")
                            if tool then
                                pcall(function() tool:Activate() end)
...
local UI_TEXT = {
    ...
    Options = {
        AutoFarm         = { Title = "Auto Farm",      Desc = "Flutua acima dos mobs e dá socos invisíveis até matá-los." },
...
if uiParent:FindFirstChild("BloxFruitsAkatUI") then
    pcall(function() uiParent.BloxFruitsAkatUI:Destroy() end)
end
...
BadgeText.Text                 = "v1.2.0"; BadgeText.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local function createToggle(parent, configKey, tabCategory)
    ...
    triggerBtn.MouseButton1Click:Connect(function()
        ...
        elseif configKey == "PlayerESP" then
            if on then UpdateESP() else ClearESP() end
...
local function AlternarConfirmacao(exibir)
    ...
    if exibir then
        ...
        if confirmBlur then
            ...
            bt:Play(); bt.Completed:Connect(function()
                if confirmBlur and confirmBlur.Size == 0 then confirmBlur:Destroy(); confirmBlur = nil end
            end)
        end
...
btnYes.MouseButton1Click:Connect(function()
    StopAutoFarm()
    Configs.AimbotPvP = false; Configs.AutoRaid = false; Configs.PlayerESP = false
    ClearESP()
...
-- PvP tab (AntiFlinch e PvPAutoBlock removidos)
